...
class GameHandlers:
    ...
    def _handle_bet_selection(self, call):
        ...
        if bet_value == 'custom':
            self.bot.send_message(call.message.chat.id,
                                  f"💎 Введите сумму ставки для {game_type}:")
    ...
    def _process_game(self, user_id, message, game_type: str, bet_amount: int):
        ...
        if not is_valid:
            self.bot.send_message(message.chat.id, f"❌ {validation_msg}",
    ...
    def _play_slots(self, user_id, message, bet_amount: int):
        """Игра в слоты"""
        loading_msg = self.bot.send_message(message.chat.id,
                                            f"🎰 Подготовка слотов...\n💎 Ставка: {bet_amount} 🪙")

        result = self.slots_machine.play(user_id, bet_amount)

        if not result['success']:
            self.bot.edit_message_text(f"❌ {result['error']}", message.chat.id,
...
🎯 Конкретное число - выигрыш x36
        ...
        self.bot.send_message(message.chat.id, bet_text,
