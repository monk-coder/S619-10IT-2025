...
class TransformerLM:
    def __init__(self, vocab_size, d_model=128, n_head=4, n_layer=3, d_ff=512, max_seq_len=128, dropout=0.1):
...
class BPETokenizer:
    ...
    def train(self, text):
        tokens = re.findall(self.pattern, text, flags=re.UNICODE)
    ...
    def decode(self, ids):
        return ''.join([k for k, v in self.vocab.items() if v in ids])
    
    def save(self, path):
        with open(path, 'w', encoding='utf-8') as f:
            json.dump({'vocab': self.vocab, 'merges': {str(k): v for k, v in self.merges.items()}, 'pattern': self.pattern}, f, ensure_ascii=False, indent=2)
    ...
    def load(cls, path):
        ...
        tok.merges = {tuple(eval(k)): v for k, v in data['merges'].items()}
        tok.pattern = data['pattern']
        tok.eos_token_id = tok.vocab['</s>']
        return tok
...
class Adam:
    def __init__(self, model, lr=3e-4, beta1=0.9, beta2=0.999, eps=1e-8):
        self.model = model
        self.lr = lr
        self.beta1 = beta1
        self.beta2 = beta2
    ...
    def _get_params(self, obj, prefix='model'):
        for attr in dir(obj):
            ...
            elif hasattr(val, '_get_trainable_params') or hasattr(val, 'weight'):
...
def load_data(path, tokenizer, seq_len):
    with open(path, 'r', encoding='utf-8') as f:
        text = f.read()
    ids = tokenizer.encode(text)
    ids.append(tokenizer.eos_token_id)
    split = int(len(ids) * 0.9)
    train_ids = np.array(ids[:split])
    val_ids = np.array(ids[split:])
    ...
    return create_dataset(train_ids), create_dataset(val_ids)
...
def train():
    ...
    (train_X, train_Y), (val_X, val_Y) = load_data(config.DATA_PATH, tokenizer, config.MAX_SEQ_LEN)
    model = TransformerLM(
    ...
    for epoch in range(config.EPOCHS):
        ...
        print(f"Epoch {epoch+1}: train_loss={avg_train:.4f}, val_loss={val_loss:.4f}")
    np.save(f"{config.SAVE_DIR}/train_losses.npy", train_losses)
    np.save(f"{config.SAVE_DIR}/val_losses.npy", val_losses)
    plot_losses(train_losses, val_losses, f"{config.LOG_DIR}/loss_curve.png")
    params = {}
