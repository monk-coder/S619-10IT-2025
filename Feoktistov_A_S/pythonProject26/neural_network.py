...
class NeuralNetwork:
    ...
    def backward(self, X, y_true, y_pred):
        ...
        Теперь сразу обновляет веса, не возвращает градиенты
        """
        m = X.shape[0]
        ...
        # НЕПОСРЕДСТВЕННОЕ ОБНОВЛЕНИЕ ВЕСОВ (главное изменение!)
        self.W1 -= self.learning_rate * dW1
        self.b1 -= self.learning_rate * db1
        self.W2 -= self.learning_rate * dW2
        self.b2 -= self.learning_rate * db2

        # Возвращаем значения потерь для логгирования (опционально)
    ...
    def train(self, X_train, y_train, X_val, y_val,
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, n_samples, batch_size):
                ...
                # Обратное распространение (теперь сразу обновляет веса!)
                # НЕ НУЖНО вызывать update_parameters отдельно!
