...
# ============================================================================
# РАЗДЕЛЕННЫЕ ФУНКЦИИ ДЛЯ ОБРАБОТКИ ШАГОВ
# ============================================================================

def _handle_profile_steps(message: types.Message, action: str, payload: dict) -> bool:
    """Обработка шагов профиля."""
...
def _handle_wishlist_steps(message: types.Message, action: str, payload: dict) -> bool:
    """Обработка шагов вишлиста."""
...
def _handle_game_creation_steps(message: types.Message, action: str, payload: dict) -> bool:
...
def _handle_code_operations_steps(message: types.Message, action: str, payload: dict) -> bool:
    """Обработка шагов операций с кодами игр."""
    def _handle_cancel_option(code: str, user_id: int, chat_id: int) -> bool:
        """Обработка отмены действия."""
        if code.lower() in {"отмена", "cancel"}:
            pop_step(user_id)
            log_action(f"step_{action}_cancel", user_id=user_id)
    ...
    # Проверка отмены для всех операций с кодами
    if _handle_cancel_option(code, message.from_user.id, message.chat.id):
        return True
    
    # Обработка конкретных операций
    if action == "join_code":
...
def _handle_question_steps(message: types.Message, action: str, payload: dict) -> bool:
    """Обработка шагов вопросов."""
...
def handle_text_step(message: types.Message, step: Dict[str, Any]) -> bool:
    """Главная функция обработки текстовых шагов (состояний)."""
    action = step["action"]
    payload = step.get("payload", {})
    
    # Профиль
    if _handle_profile_steps(message, action, payload):
        return True
    
    # Вишлист
    if _handle_wishlist_steps(message, action, payload):
        return True
    
    # Создание игры
    if _handle_game_creation_steps(message, action, payload):
        return True
    
    # Операции с кодами (присоединение, выход, участники, жеребьёвка)
    if _handle_code_operations_steps(message, action, payload):
        return True
    
    # Вопросы
    if _handle_question_steps(message, action, payload):
        return True
