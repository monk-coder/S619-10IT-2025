...
# Добавим эти импорты в начало train.py
from sklearn.metrics import confusion_matrix
import seaborn as sns


# Добавим эту функцию в train.py (перед main())
def plot_all_results(model, X_val, y_val_labels, history):
    """
    Построение всех графиков: обучения, матрицы ошибок и примеров предсказаний
    """
    # 1. График обучения (уже есть)
    fig1, axes1 = plt.subplots(1, 2, figsize=(15, 5))

    axes1[0].plot(history['train_loss'], label='Train Loss')
    axes1[0].plot(history['val_loss'], label='Validation Loss')
    ...
    axes1[1].plot(history['train_accuracy'], label='Train Accuracy')
    axes1[1].plot(history['val_accuracy'], label='Validation Accuracy')
    ...
    for i, idx in enumerate(sample_indices):
        ...
        axes3[i].imshow(image, cmap='gray')
        axes3[i].axis('off')
        ...
        axes3[i].set_title(f'True: {true_label}\nPred: {pred_label}\nProb: {probs[pred_label]:.2f}',
    ...
    print(f"Матрица ошибок сохранена в confusion_matrix.png")
    print(f"Примеры предсказаний сохранены в sample_predictions.png")
