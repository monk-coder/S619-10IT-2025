...
def load_corpus(path: str) -> List[str]:
    """Load corpus from file, split into lines/documents."""
...
def evaluate_tokenizer(tokenizer: BPETokenizer, val_corpus: List[str]) -> dict:
    """Evaluate tokenizer on validation corpus."""
    lengths = []
    ...
    for text in val_corpus:
        ...
        try:
            ...
            if reconstructed != text:
                # Be lenient with minor whitespace differences at boundaries
                if reconstructed.strip() != text.strip():
                    reconstruction_errors += 1
                    if len(mismatched_examples) < 5:  # Save first 5 mismatches
                        mismatched_examples.append((text, reconstructed))
                else:
                    # Only boundary whitespace difference - acceptable
                    pass
    ...
    metrics = {
        ...
        "mismatched_examples": mismatched_examples[:5]  # Store up to 5 examples
    }
...
def run_experiment(train_corpus: List[str], val_corpus: List[str], merge_configs: List[int]):
    ...
    for num_merges in merge_configs:
        ...
        # Show reconstruction errors if any
...
def plot_results(results: dict):
    """Plot comparison of metrics across different num_merges values."""
...
def main():
    # Configuration
    DATA_PATH = "data.txt"
    MERGE_CONFIGS = [0, 2000, 8000]  # Experiment with different merge counts
    ...
    # Demonstrate encode/decode usage
    print("\n" + "="*60)
    print("🔍 Demonstration of encode/decode:")
    print("="*60)
    
    # Load best tokenizer (highest merges)
