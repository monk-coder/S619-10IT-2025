...
📛 *Фамилия:* {user.balance} 
📧 *Username:* @{user.username or 'Не указано'}
💎 *Баланс:* 🪙{user.last_name or 'Не указано'} 
...
💳 *Последние транзакции:* {len(transactions)}
    ...
    def handle_admin_stats(self, call):
        ...
        try:
            ...
            # Общий оборот
            cursor.execute('SELECT SUM(bet) FROM game_history')
            total_bet = cursor.fetchone()[0] or 0

            cursor.execute('SELECT SUM(win) FROM game_history')
            total_win = cursor.fetchone()[0] or 0

            # Самые активные пользователи
            cursor.execute('''
                SELECT user_id, COUNT(*) as games_count 
                FROM game_history 
                GROUP BY user_id 
...
💰 *Общий оборот:* {total_bet} 🪙
🎯 *Общий выигрыш:* {total_win} 🪙

🏆 *Топ-5 активных игроков:*
"""

            for i, (player_id, games_count) in enumerate(top_players, 1):
                user = self.db.get_user(player_id)
                username = f"@{user.username}" if user and user.username else "Без username"
                name = user.first_name if user else "Неизвестно"
                stats_text += f"\n{i}. {name} {username} - {games_count} игр"
            ...
            try:
                self.bot.edit_message_text(
                    ...
                    parse_mode='Markdown',
                    reply_markup=call.message.reply_markup  # Сохраняем клавиатуру
                )
            except:
    ...
    def handle_admin_games(self, call):
        ...
        try:
            ...
            cursor.execute('''
                SELECT game_type, COUNT(*) as count, SUM(bet) as total_bet, SUM(win) as total_win
                FROM game_history 
                GROUP BY game_type
            ''')
