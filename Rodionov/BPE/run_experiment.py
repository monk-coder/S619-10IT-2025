...
def create_sample_corpus(filepath: str = "data/sample_corpus.txt"):
    ...
    # Более разнообразный корпус для обучения BPE
...
"""
    ...
    # Разбиваем на строки и возвращаем
...
def train_and_evaluate(num_merges: int, train_data: List[str], val_data: List[str],
    ...
    if not val_data:
        ...
        # Используем train_data для тестирования
...
def compare_experiments(merge_values: List[int], results_dir: str = "results"):
    ...
    try:
        ...
        # График 3: 99-й процентиль длины vs num_merges
        ax = axes[1, 0]
        ...
        # График 4: Доля очень длинных последовательностей
        ax = axes[1, 1]
...
def run_interactive_demo():
    ...
    # Проверяем, есть ли папка models
    ...
    # Выбираем модель с наибольшим num_merges
...
def main():
    ...
    MERGE_VALUES = [0, 500, 2000]  # Меньше значений для быстрого теста
    ...
    # 3. Обучение и оценка для разных num_merges
    print("\n3. ОБУЧЕНИЕ МОДЕЛЕЙ")
    print("-" * 40)
