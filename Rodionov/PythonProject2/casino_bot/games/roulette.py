...
class RouletteGame:
    ...
    def check_win(self, bet_type, bet_value, winning_number, winning_color):
        ...
        logger.info(
            f"🎯 Проверка выигрыша: type={bet_type}, value={bet_value}, win_num={winning_number}, win_color={winning_color}")
    ...
    def get_multiplier(self, bet_type):
        """Получение множителя для типа ставки - ИСПРАВЛЕННЫЕ ЗНАЧЕНИЯ"""
        multipliers = {
            'color': 2,
            'even_odd': 2,
            'specific': 36,
            'dozen': 3  # ИСПРАВЛЕНО: для дюжины множитель 3
        }
