...
def simulate_coin_collection(days=365, max_denomination=10000):
    """Симуляция коллекционирования монет Алисы"""
    collected = set()
    discarded_count = 0
...
def get_simulation_period():
    while True:
        try:
            years_input = input("Введите количество лет для симуляции жизни (например, 3.5): ")
...
""")
    ...
    if len(collected_coins) > 20:
        first_twenty = sorted(list(collected_coins))[:20]
        collection_preview = f"Первые 20 номиналов: {first_twenty}\n... и еще {len(collected_coins) - 20} монет"
    else:
        collection_preview = f"Номиналы собранных монет: {sorted(list(collected_coins))}"
    
    print(f"""
КОЛЛЕКЦИЯ МОНЕТ АЛИСЫ (результаты за 365 дней):

Всего дней: {DAYS}
Диапазон номиналов: 1-{MAX_DENOMINATION}
Уникальных монет собрано: {len(collected_coins)}
Монет выброшено (дубликаты): {discarded_count}
