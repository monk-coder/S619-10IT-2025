...
def register():
    ...
    try:
        ...
        if existing_user: return jsonify({'error': 'Пользователь уже существует'}), 400
        
        password_hash = hash_password(password)
        api_key = secrets.token_hex(32)
        
        cursor = db.execute('INSERT INTO users (username, email, password_hash, api_key) VALUES (?, ?, ?, ?)', (username, email, password_hash, api_key))
        ...
        return jsonify({
            ...
            'user': {'id': user_id, 'username': username, 'email': email, 'api_key': api_key}
        }), 201
...
def add_to_wishlist():
    ...
    try:
        existing = db.execute('SELECT id FROM wishlists WHERE user_id = ? AND country_name = ?', (user['id'], country_data['name'])).fetchone()
        
        if existing: return jsonify({'error': 'Страна уже в вишлисте'}), 400
        
        db.execute('INSERT INTO wishlists (user_id, country_name, country_data) VALUES (?, ?, ?)', (user['id'], country_data['name'], json.dumps(country_data)))
        db.commit()
...
def remove_from_wishlist(country_name):
    ...
    try:
        ...
        if result.rowcount > 0: return jsonify({'message': 'Страна удалена из вишлиста'})
        else: return jsonify({'error': 'Страна не найдена в вишлисте'}), 404
