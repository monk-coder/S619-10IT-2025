class Wishlist {
    ...
    static async get() {
        ...
        try {
            const response = await fetch(`${this.baseURL}/wishlist`, {
                headers: {'Authorization': `Bearer ${AuthService.getApiKey()}`, 'Content-Type': 'application/json'}
            });
    ...
    static async render() {
        ...
        if (!AuthService.isLoggedIn()) {
            list.innerHTML = `<div class="empty-wishlist">
                <i class="fas fa-user-lock"></i><h3>Войдите в систему</h3>
                <p>Для просмотра вашего вишлиста необходимо войти в систему.</p>
                <button onclick="AuthUI.showLoginModal()"><i class="fas fa-sign-in-alt"></i> Войти</button>
            </div>`;
            return;
        }
        ...
        try {
            ...
            if (wishlist.length === 0) {
                list.innerHTML = `<div class="empty-wishlist">
                    <i class="fas fa-compass"></i><h3>Ваш вишлист пуст</h3>
                    <p>Начните поиск стран и добавляйте их в свой список!</p>
                    <button onclick="switchTab('search')"><i class="fas fa-search"></i> Найти страны</button>
                </div>`;
                return;
            }

            list.innerHTML = wishlist.map(country => `<div class="wishlist-item">
